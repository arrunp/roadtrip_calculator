...
public class DijkstraSP {
     ...
     * @throws IllegalArgumentException unless {@code 0 <= v < V}
     */
    public Iterable<DirectedEdge> pathTo(int v) throws IOException {
    	java.util.List<NumbCity> list = DepthFirstDirectedPaths.listofcities();
        validateVertex(v);
        ...
        /*
        for (DirectedEdge e = edgeTo[v]; e != null; e = edgeTo[e.from()]) {
            for (NumbCity city : list) {
            	if(city.getcityNum() == e.to()) {
            		path.push(city.getcityName());
            	}
            }
            currentedge = e.from(); 
        }
        ...
        for(NumbCity x: list) {
        	if(x.getcityNum() == currentedge)
        		path.push(x.getcityName());
        }
        */
        return path;
    }
